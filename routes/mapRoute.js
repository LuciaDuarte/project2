...
function requireLogin(req, res, next) {
  // we create a middleware function, to pass it in the route.
  if (req.session.currentUser) {
    // If this exists, means we are authenticated.
    next();
  } else {
    res.redirect('/login');
  }
  ...
  User.find({ userType: 'needy' })
    ...
    .then(needyUsersFromDB => {
      ...
      res.render('map/helper-map', {
        user: userStr,
        needyUsersFromDB: needyUsersFromDB,
        markers: needyUsersFromDBString
  ...
  User.find({ userType: 'helper' })
    ...
    .then(helpersUsersFromDB => {
      ...
      res.render('map/needy-map', {
        user: userStr,
        helpersUsersFromDB: helpersUsersFromDB,
        markers: helpersUsersFromDBString
      });
    })
    .catch(err => console.log('Error:', err));
});

...
router.post('/signup', (req, res) => {
  ...
  axios
    .get(
      `https://maps.googleapis.com/maps/api/geocode/json?address=${postCode}+${address}+${city}&key=${process.env.GOOGLE_MAPS_API_KEY}`
    )
    .then(responsegeo => {
    ...
    .then(() => {
      ...
      if (userType === 'helper') {
        ...
        HelpType.find({ name: helpType, subServices: subServices }) // subst when created
          ...
          .catch(err =>
            console.log('An error occured while creating a helper:', err)
          );
      } else {
        switch (needType) {
          case 'food':
        ...
        NeedType.find({ name: needType })
          ...
          .catch(err =>
            console.log('An error occured while creating a needer:', err)
          );
      }
    });
});
